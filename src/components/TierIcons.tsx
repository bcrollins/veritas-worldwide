...
export function PillarIcon({ className = 'w-5 h-5' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
...
export function MoneyIcon({ className = 'w-5 h-5' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
...
export function ScaleIcon({ className = 'w-5 h-5' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 3v18M3 7l3 5c0 1.66 1.34 3 3 3s3-1.34 3-3l3-5M12 7l3 5c0 1.66 1.34 3 3 3s3-1.34 3-3l3-5" />
    </svg>
  )
}
...
export function FlameIcon({ className = 'w-5 h-5' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 23c-3.866 0-7-3.134-7-7 0-3.5 2.5-6 4-7.5 1.5 1 2 2.5 2 2.5s1.5-2.5 1-5c3 1.5 5 4.5 5 7.5a4.978 4.978 0 01-2 4" />
      <path d="M12 23a3 3 0 01-3-3c0-1.5 1-2.5 1.5-3 .5.5 1 1.5 1 1.5s.5-1 .5-2c1 .5 2 1.5 2 3a3 3 0 01-2 2.5" />
    </svg>
  )
}
...
export function TierIcon({ name, className = 'w-5 h-5' }: { name: string; className?: string }) {
  const icons: Record<string, React.FC<{ className?: string }>> = {
    signal: SignalIcon, search: SearchIcon, pillar: PillarIcon,
    money: MoneyIcon, scale: ScaleIcon, shield: ShieldIcon,
    chart: ChartIcon, flame: FlameIcon, warning: WarningIcon,
    plane: PlaneIcon, globe: GlobeIcon, video: VideoIcon,
    camera: CameraIcon, file: FileTextIcon, check: CheckIcon,
  }
  const Icon = icons[name]
  return Icon ? <Icon className={className} /> : <span className={className} />
}
