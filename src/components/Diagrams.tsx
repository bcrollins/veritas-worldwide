/**
 * Professional SVG diagrams for Veritas Worldwide Press
    ...
    <figure className="my-10 max-w-2xl mx-auto">
      <div className="overflow-hidden rounded-sm border border-border bg-surface p-6">
        <svg viewBox="0 0 700 520" className="w-full h-auto" role="img" aria-label="Diagram showing how three asset managers — Vanguard, BlackRock, and State Street — are top shareholders in all six major media corporations">
          ...
          {/* Title */}
          <text x="350" y="30" textAnchor="middle" className="font-sans" style={{ fontSize: '14px', fontWeight: 700, fill: '#1A1A1A', letterSpacing: '0.1em' }}>MEDIA OWNERSHIP STRUCTURE</text>
          <text x="350" y="48" textAnchor="middle" className="font-sans" style={{ fontSize: '10px', fill: '#999', letterSpacing: '0.05em' }}>THREE ASSET MANAGERS → SIX CORPORATIONS → 90% OF AMERICAN MEDIA</text>
          ...
          {[
            ...
            <g key={mc.label}>
              ...
              <text x={mc.x} y={184} textAnchor="middle" style={{ fontSize: '8px', fontWeight: 700, fill: '#1A1A1A', letterSpacing: '0.06em' }}>{mc.label}</text>
              <text x={mc.x} y={199} textAnchor="middle" style={{ fontSize: '7px', fill: '#666' }}>{mc.sub}</text>
            </g>
          ))}
          {/* Arrow to output */}
          ...
          {/* Footer note */}
          <text x="350" y="320" textAnchor="middle" style={{ fontSize: '8px', fill: '#999', fontStyle: 'italic' }}>Source: SEC filings, Harvard Future of Media Project, 2021–2025</text>
        </svg>
      </div>
    ...
    <figure className="my-10 max-w-2xl mx-auto">
      <div className="overflow-hidden rounded-sm border border-border bg-surface p-6">
        <svg viewBox="0 0 650 440" className="w-full h-auto" role="img" aria-label="Diagram showing the structure of the Federal Reserve System — a hybrid public-private institution">
          {/* Title */}
          <text x="325" y="28" textAnchor="middle" className="font-sans" style={{ fontSize: '13px', fontWeight: 700, fill: '#1A1A1A', letterSpacing: '0.1em' }}>FEDERAL RESERVE SYSTEM STRUCTURE</text>
          <text x="325" y="46" textAnchor="middle" className="font-sans" style={{ fontSize: '9px', fill: '#999', letterSpacing: '0.05em' }}>A HYBRID PUBLIC-PRIVATE INSTITUTION — ESTABLISHED 1913</text>
          ...
          <text x={325} y={112} textAnchor="middle" style={{ fontSize: '7px', fill: '#666', fontStyle: 'italic' }}>Government entity — but independent of executive control</text>
          {/* Arrow down */}
          <line x1="325" y1="120" x2="325" y2="150" stroke="#1A1A1A" strokeWidth={1} strokeDasharray="4,3" />
          ...
          <text x={325} y={202} textAnchor="middle" style={{ fontSize: '7px', fill: '#666', fontStyle: 'italic' }}>Not government agencies — issue stock to member banks, pay dividends</text>

          {/* Arrow down */}
          <line x1="325" y1="210" x2="325" y2="240" stroke="#1A1A1A" strokeWidth={1} strokeDasharray="4,3" />

          {/* Member Banks */}
          <rect x={125} y={245} width={400} height={45} rx={4} fill="#1A1A1A" opacity={0.05} stroke="#1A1A1A" strokeWidth={1} />
          <text x={325} y={265} textAnchor="middle" style={{ fontSize: '9px', fontWeight: 700, fill: '#1A1A1A', letterSpacing: '0.08em' }}>MEMBER COMMERCIAL BANKS</text>
          <text x={325} y={280} textAnchor="middle" style={{ fontSize: '8px', fill: '#666' }}>JPMorgan Chase · Citibank · Goldman Sachs · Bank of America · Wells Fargo</text>
          {/* Key powers sidebar */}
          ...
          <text x={45} y={348} style={{ fontSize: '8px', fill: '#333' }}>• Sets interest rates (Federal Funds Rate)</text>
          <text x={45} y={363} style={{ fontSize: '8px', fill: '#333' }}>• Creates money through open market operations</text>
          <text x={45} y={378} style={{ fontSize: '8px', fill: '#333' }}>• Regulates member banks</text>
          <text x={45} y={393} style={{ fontSize: '8px', fill: '#333' }}>• Lender of last resort</text>
          <text x={45} y={408} style={{ fontSize: '8px', fill: '#333' }}>• Never been independently audited</text>
          ...
          <text x={355} y={348} style={{ fontSize: '8px', fill: '#333' }}>• Created by the Federal Reserve Act of 1913</text>
          <text x={355} y={363} style={{ fontSize: '8px', fill: '#333' }}>• Not a government agency, not fully private</text>
          <text x={355} y={378} style={{ fontSize: '8px', fill: '#333' }}>• Dollar has lost 97% of value since 1913</text>
          <text x={355} y={393} style={{ fontSize: '8px', fill: '#333' }}>• Profits returned to U.S. Treasury (after expenses)</text>
          <text x={355} y={408} style={{ fontSize: '8px', fill: '#333' }}>• Chairman testifies to Congress, not controlled by it</text>

          {/* Source */}
          <text x="325" y="438" textAnchor="middle" style={{ fontSize: '7px', fill: '#999', fontStyle: 'italic' }}>Source: Federal Reserve Act (12 USC §221), Federal Reserve Board publications, Congressional Research Service</text>
        </svg>
      </div>
      <figcaption className="mt-3 px-1">
    ...
    <figure className="my-10 max-w-2xl mx-auto">
      <div className="overflow-hidden rounded-sm border border-border bg-surface p-6">
        <svg viewBox="0 0 650 430" className="w-full h-auto" role="img" aria-label="Diagram showing how BlackRock, Vanguard, and State Street are top shareholders across every major sector of the American economy">
          <text x="325" y="25" textAnchor="middle" style={{ fontSize: '13px', fontWeight: 700, fill: '#1A1A1A', letterSpacing: '0.1em' }}>CROSS-SECTOR INSTITUTIONAL OWNERSHIP</text>
          <text x="325" y="43" textAnchor="middle" style={{ fontSize: '9px', fill: '#999', letterSpacing: '0.05em' }}>THE SAME THREE FIRMS ARE TOP SHAREHOLDERS IN EVERY MAJOR INDUSTRY</text>
          ...
          {sectors.map((s) => (
            <g key={s.label}>
              ...
              <text x={340} y={s.y + 23} textAnchor="middle" style={{ fontSize: '8px', fill: '#666' }}>{s.companies}</text>
            </g>
          ))}
          <text x="325" y="425" textAnchor="middle" style={{ fontSize: '7px', fill: '#999', fontStyle: 'italic' }}>Source: SEC 13F filings, Bloomberg, 2024–2025</text>
        </svg>
      </div>
      <figcaption className="mt-3 px-1">
