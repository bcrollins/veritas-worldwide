...
export default function CookieConsent() {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-[100] bg-obsidian/95 backdrop-blur-md text-white px-3 py-2.5 sm:px-6 sm:py-4 no-print safe-area-pb"
      role="dialog"
      aria-label="Cookie consent"
      data-testid="cookie-consent-banner"
    >
      <div className="max-w-5xl mx-auto flex flex-row items-center gap-2.5 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          <button
            ...
            className="font-sans text-[10px] sm:text-xs font-semibold tracking-[0.08em] sm:tracking-[0.1em] uppercase px-3 py-2 sm:px-4 sm:py-2.5 min-h-[44px] border border-white/20 text-white/70 rounded-sm hover:border-white/40 hover:text-white transition-colors"
          >
            Decline
          </button>
          <button
            onClick={handleAccept}
            className="font-sans text-[10px] sm:text-xs font-semibold tracking-[0.08em] sm:tracking-[0.1em] uppercase px-3.5 py-2 sm:px-5 sm:py-2.5 min-h-[44px] bg-crimson text-white rounded-sm hover:bg-crimson-dark transition-colors"
          >
            Accept
          </button>
        </div>
