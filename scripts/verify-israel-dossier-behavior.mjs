...
async function runInteractiveChecks(browser) {
  ...
  try {
    ...
    assert(Array.isArray(corpus?.incidents) && corpus.incidents.length >= 89, 'corpus.json incidents too few')
    ...
    assert(Array.isArray(corpus?.legalCases) && corpus.legalCases.length >= 18, 'corpus.json legalCases too few')
