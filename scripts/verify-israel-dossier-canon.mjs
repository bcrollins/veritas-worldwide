...
// Visual Investigations densify (NYT VI / AP method floors)
{
  ...
  assert(cards >= 1172, `VI pack should have ≥50 visual cards, got ${cards}`)
...
// Soft-floor companion tracks video densify (lag-aware live verify)
{
  ...
  assert(typeof sf.incidentCount === 'number' && sf.incidentCount >= 5106, `soft-floor incidentCount too low: ${sf.incidentCount}`)
  ...
  assert(sf.visualInvestigations.withVideo >= 1166, `soft-floor withVideo floor ≥54, got ${sf.visualInvestigations.withVideo}`)
}
