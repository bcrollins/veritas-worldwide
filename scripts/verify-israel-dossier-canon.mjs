...
// Visual Investigations densify (NYT VI / AP method floors)
{
  ...
  assert(cards >= 539, `VI pack should have ≥50 visual cards, got ${cards}`)
...
// Soft-floor companion tracks video densify (lag-aware live verify)
{
  ...
  assert(sf.visualInvestigations.withVideo >= 538, `soft-floor withVideo floor ≥54, got ${sf.visualInvestigations.withVideo}`)
}
