...
// Visual Investigations densify (NYT VI / AP method floors)
{
  ...
  assert(cards >= 2129, `VI pack should have ≥50 visual cards, got ${cards}`)
...
// Soft-floor companion tracks video densify (lag-aware live verify)
{
  ...
  assert(typeof sf.incidentCount === 'number' && sf.incidentCount >= 9039, `soft-floor incidentCount too low: ${sf.incidentCount}`)
  ...
  assert(sf.visualInvestigations.withVideo >= 2153, `soft-floor withVideo floor ≥54, got ${sf.visualInvestigations.withVideo}`)
}
