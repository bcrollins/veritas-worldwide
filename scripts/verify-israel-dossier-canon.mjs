...
assert(page.includes('useSearchParams'), 'dossier page missing shareable deep-link query params')
assert(page.includes('downloadIncidentsCsv') || page.includes('Export filtered CSV'), 'dossier page missing CSV export')
assert(page.includes('Copy share link') || page.includes('copyShareUrl'), 'dossier page missing share-link copy control')
assert(page.includes('Export money-trail CSV') || page.includes('downloadMoneyTrailCsv'), 'dossier page missing money-trail CSV export')
assert(page.includes('Export timeline CSV') || page.includes('downloadTimelineCsv'), 'dossier page missing timeline CSV export')
assert(read('src/data/israelDossierActors.ts').includes("profileId: 'bernie-sanders'"), 'actors graph missing counter-enablement Sanders node')
assert(read('src/data/israelDossierActors.ts').includes("profileId: 'rashida-tlaib'"), 'actors graph missing counter-enablement Tlaib node')
assert(read('src/pages/HomePage.tsx').includes('/israel-dossier'), 'homepage missing Israel dossier entry CTA')
assert(read('scripts/verify-auth-cache-browser.mjs').includes('Scroll restore is async'), 'auth-cache browser verifier missing scroll-restore poll harden')
assert(fs.existsSync(path.join(root, 'public/israel-dossier/corpus.json')), 'missing public/israel-dossier/corpus.json corpus export')
assert(read('public/israel-dossier/corpus.json').includes('"schemaVersion"'), 'corpus.json missing schemaVersion')
assert(read('public/israel-dossier/corpus.json').includes('"incidents"'), 'corpus.json missing incidents array')
assert(read('package.json').includes('export-israel-dossier-corpus'), 'package.json missing corpus export script')
assert(page.includes('corpus.json'), 'dossier page missing corpus.json download CTA')
assert(page.includes('actorCategory') || page.includes('All actor categories'), 'dossier page missing actor category filter')
assert(read('src/pages/TopicPage.tsx').includes('/israel-dossier'), 'topic page missing Israel dossier CTA wiring')
assert(read('src/data/topicHubs.json').includes('Interactive evidence engine') || read('src/data/topicHubs.json').includes('israel-dossier'), 'israel-policy topic hub not linked to dossier')
assert(read('scripts/prerender.mjs').includes('/israel-dossier/corpus.json'), 'prerender sitemap missing corpus.json entry')
assert(historyPack.includes('first-intifada-civilian-pattern'), 'history pack missing First Intifada pattern record')
assert(historyPack.includes('grapes-of-wrath-1996'), 'history pack missing Grapes of Wrath campaign record')
assert(historyPack.includes('operation-cast-lead-school-strikes'), 'history pack missing Cast Lead school/shelter strikes')
assert(historyPack.includes('operation-protective-edge-unrwa-shelters'), 'history pack missing Protective Edge UNRWA shelters')
assert(read('src/pages/SearchPage.tsx').includes('matchingDossierSurfaces'), 'search page missing Israel Dossier cross-surface matches')
...
// Visual Investigations densify (NYT VI / AP method floors)
{
  ...
  assert(cards >= 2309, `VI pack should have ≥50 visual cards, got ${cards}`)
...
// Soft-floor companion tracks video densify (lag-aware live verify)
{
  ...
  assert(typeof sf.incidentCount === 'number' && sf.incidentCount >= 10134, `soft-floor incidentCount too low: ${sf.incidentCount}`)
  ...
  assert(sf.visualInvestigations.withVideo >= 2423, `soft-floor withVideo floor ≥54, got ${sf.visualInvestigations.withVideo}`)
}
