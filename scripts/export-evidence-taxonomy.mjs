#!/usr/bin/env node
/**
 * Export machine-readable evidence taxonomy for researchers + GEO.
 * Source of truth: src/data/evidenceTiers.ts (mirrored here for Node without TS runtime).
...
const outPath = path.join(root, 'public', 'evidence-taxonomy.json')
...
const payload = {
  ...
  publisherUrl: 'https://veritasworldwide.com',
  contact: 'rights@veritasworldwide.com',
  methodologyUrl: 'https://veritasworldwide.com/methodology',
  license: 'CC BY-NC-SA 4.0 (see https://veritasworldwide.com/terms)',
  scholarlyOrder: Object.keys(scholarly),
  scholarlyTiers: scholarly,
  volumeILegacyTiers: {
    verified: {
      id: 'verified',
      label: 'Verified',
      mapsToScholarly: 'verified',
      note: 'Primary-source / multi-source documentation in Volume I sense.',
    },
    circumstantial: {
      id: 'circumstantial',
      label: 'Circumstantial',
      mapsToScholarly: 'circumstantial',
      alsoMayMapTo: ['interpretive'],
      note: 'Documented facts with interpretive connection.',
    },
    disputed: {
      id: 'disputed',
      label: 'Disputed',
      mapsToScholarly: 'contested',
      alsoMayMapTo: ['speculative', 'literary_theological'],
      note: 'Volume I disputed ≈ scholarly Contested; some rows Speculative or Literary/Theological.',
    },
  },
  sourceHierarchy: [
    { tier: 1, label: 'Government & Legal Records' },
    { tier: 2, label: 'Institutional Records' },
    { tier: 3, label: 'Investigative Journalism' },
    { tier: 4, label: 'Academic & Scholarly Works' },
    { tier: 5, label: 'Secondary Reporting & Analysis' },
  ],
}

fs.writeFileSync(outPath, `${JSON.stringify(payload, null, 2)}\n`, 'utf8')
console.log(`[export-evidence-taxonomy] wrote ${outPath}`)
