...
6. `/methodology` + `/sources` — FAQ, local-only timeline link
7. `/researcher/timeline` — noindex, local-only, export watermark
8. `/volume-ii` — noindex scaffold
9. Soft-404: `/chapter/not-a-real-slug` → 404 + noindex
10. OPSEC: `/bernie` → noindex meta + X-Robots
